...
The autonomous local foundation reuses Guardian, Crossref, Douban, Letterboxd and YouTube/Bilibili
adapters only when the existing packet has a typed evidence gap. A packet that was already sufficient
makes no planner or provider call. A recovered packet must include at least two independent origins;
several excerpts from one domain cannot satisfy that rule. `fetch_letterboxd_reviews` uses the
