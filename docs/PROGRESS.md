...
1. Replaced the simulated CSS room with a real 6.1 MB GLB generated by a deterministic Blender
   script, including a walk-in shell, three fitted shelf walls, smoked-oak boards, blackened steel,
   brass rails, carpet, ceiling lights, entrance framing and populated ambient archive rows.
2. Added a pinned, locally served Three.js WebGL runtime and GLTF loader; the closet has no CDN or
...
5. Added generated paper spines, physical brass shelf plaques, hover pull-out animation, ray-cast case
   picking and selected-film highlighting; selecting a 3D case rebuilds both the edition and closet.
...
- the production GLB loaded successfully from FastAPI into a 1237 × 907 WebGL canvas;
- visible browser checks confirmed the entrance view, centre wall, side aisle and close shelf detail;
- two Walk in actions changed the HUD from `ENTRANCE` to `MID-ROOM`, while a real pointer drag turned
  into the side aisle;
...
- the GLB adds roughly 6.1 MB to the first closet load; it is cached by the browser, and a future model
  optimisation pass can merge ambient-case meshes and apply geometry compression if load time becomes
  material on slower devices.
...
**Automated verification:** 75 tests passing

| Area | Status | Current evidence |
|---|---|---|
| Film discovery | Complete | Wikidata identity search, attributed dossier enrichment and a real Blender/Three.js walk-in related-film closet |
