/**
 * FirstRoll project-local adaptation of Pi's agent discovery example.
...
function loadAgentsFromDir(dir: string, source: "user" | "project"): AgentConfig[] {
	...
	for (const entry of entries) {
		...
		// Repo-controlled prompts must be physical files. User-level agents retain
		// Pi's standard symlink installation flow and are trusted as user configuration.
		if (source === "project" && !entry.isFile()) continue;
		if (source === "user" && !entry.isFile() && !entry.isSymbolicLink()) continue;
...
function isPhysicalDirectory(p: string): boolean {
	try {
...
function findNearestProjectAgentsDir(cwd: string): string | null {
	...
	while (true) {
		...
		if (isPhysicalDirectory(configDir) && isPhysicalDirectory(candidate)) return candidate;
